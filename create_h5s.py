...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir,
    ...
    for root, _, files in os.walk(source):
        for file in files:
            if fnmatch.fnmatch(file, f"*.svs") or fnmatch.fnmatch(file, f"*.tif"):
    ...
    for i in range(total):
        ...
        WSI_object = WholeSlideImage(full_path[i])
        ...
        if patch:
            # app_magn = wsi.properties['aperio.AppMag']

            if ".tif" in full_path[i]:
                level_count = wsi.properties['openslide.level-count']
                custom_downsample = 2 # 40x

            else:

                level_count = wsi.properties['openslide.level-count']
                if level_count == '4':  # if app_magn == '40': # 40x
                    custom_downsample = 2
                elif level_count == '3':  # else: # 20x
                    custom_downsample = 1
            current_patch_params.update({'patch_level': patch_level, 'patch_size': patch_size, 'step_size': step_size,
                                         'save_path': patch_save_dir, 'custom_downsample': custom_downsample})
            file_path, patch_time_elapsed = patching(WSI_object=WSI_object, **current_patch_params)

        if file_path == None:  # No countours/no stitches found -> nothing to patch
            continue
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--source', default="./data/imagenes_de_prueba", type = str,
    # 					help='path to folder containing raw wsi image files')
    parser.add_argument('--source', default="E:/CLARIFY BREAST CANCER DATABASE/WSIS CLARIFY FULL BCDB RECONSTRUCTED", type=str,
                        help='path to folder containing raw wsi image files')
    ...
    # parser.add_argument('--save_dir', default="./data/parches_de_prueba", type = str,
    # 					help='directory to save processed data')
    parser.add_argument('--save_dir', default="F:/CLAUDIO/BREAST_CANCER_DATASETS/CLARIFY BREAST CANCER DATABASE NOV2023/Results CLAM/h5s", type=str,
                        help='directory to save processed data')
