...
import numpy as np
with os.add_dll_directory('C:/Users/clferma1/Downloads/openslide-win64-20221217/bin'):
    import openslide
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            # asset_dict is dict with coordinate that are valid (not in holes) for the patching
            if len(asset_dict) > 0:
                if init:
