...
def savePatchIter_bag_hdf5(patch):
    ...
    #TODO: Check RGB Levels to see if the patch is fully red, green or blue
    img_patch_arr = img_patch[0]
    #plt.imshow(img_patch_arr)
    #plt.show()

    # Example usage:
    red_threshold = 0.05  # Adjust as needed
    green_threshold = 0.05  # Adjust as needed
    blue_threshold = 0.05  # Adjust as needed

    pen_marker_bool, red_dominance, green_dominance, blue_dominance = is_pen_marker_patch(img_patch_arr, red_threshold, green_threshold, blue_threshold)
    pen_folder = "E:/CLARIFY BREAST CANCER DATABASE/Results CLAM/h5s/discarded_patches_marker"
    if pen_marker_bool:
        print("patch contains marker, not being saved in h5 file!")
        patch = Image.fromarray(img_patch_arr.astype('uint8'))
        patch.save(os.path.join(pen_folder, "marker", generate_random_string(10) + ".png"))
    else:
        #patch = Image.fromarray(img_patch_arr.astype('uint8'))
        #patch.save(os.path.join(pen_folder, "not_marker", generate_random_string(10) + ".png"))

        # Save patches only if not_marker
...
def is_pen_marker_patch(img_patch, red_threshold, green_threshold, blue_threshold):
    """
    Check if an image patch is likely a pen marker artifact.
    """
    # Define color threshold ranges for red, green, and blue.
    red_range = (np.array([200, 0, 0]), np.array([255, 100, 100]))
    green_range = (np.array([0, 200, 0]), np.array([100, 255, 100]))
    blue_range = (np.array([0, 0, 200]), np.array([100, 100, 255]))

    # Count pixels in each color range.
    red_pixels = np.sum(np.all((img_patch >= red_range[0]) & (img_patch <= red_range[1]), axis=-1))
    green_pixels = np.sum(np.all((img_patch >= green_range[0]) & (img_patch <= green_range[1]), axis=-1))
    blue_pixels = np.sum(np.all((img_patch >= blue_range[0]) & (img_patch <= blue_range[1]), axis=-1))

    # Calculate color dominance percentages.
    total_pixels = img_patch.shape[0] * img_patch.shape[1]
    red_dominance = red_pixels / total_pixels
    green_dominance = green_pixels / total_pixels
    blue_dominance = blue_pixels / total_pixels

    # Check if any color dominance exceeds the specified thresholds.
    if (red_dominance > red_threshold) or (green_dominance > green_threshold) or (blue_dominance > blue_threshold):
        return True, red_dominance, green_dominance, blue_dominance  # Exclude the patch
    else:
        return False, red_dominance, green_dominance, blue_dominance  # Include the patch
...
def initialize_hdf5_bag(first_patch, save_coord=False):
    ...
    # blue_threshold = 0.05  # Adjust as needed
    #
    # pen_marker_bool = is_pen_marker_patch(img_patch_arr, red_threshold, green_threshold, blue_threshold)
    # pen_folder = "./data/parches_de_prueba/pen_marker_debug_0.05"
    # if pen_marker_bool:
    #     patch = Image.fromarray(img_patch_arr.astype('uint8'))
    #     patch.save(os.path.join(pen_folder, "marker", generate_random_string(10) + ".png"))
    # else:
